...
export async function monitorsRoutes(app: FastifyInstance) {
  ...
  app.get('/api/route-stats', { preHandler: authenticate }, async (request, reply) => {
    ...
    // Ausência de dado real é resposta legítima, e a tela de cadastro
    // precisa distinguir isso de "é barato" — 404 em vez de devolver
    // números inventados. Ver _local-bdr-policy-016.
    if (!stats) {
      return reply.status(404).send({ error: 'Sem preço real observado para esta rota no momento.' });
    }
    return stats;
  });
