...
interface EmailModalProps {
  ...
  onClose: () => void;
  onUnsubscribe?: (monitorId: string) => void;
}

export default function EmailModal({ notification, onClose, onUnsubscribe }: EmailModalProps) {
        ...
        <div className="bg-white text-zinc-800 p-8 overflow-y-auto flex-1 font-sans animate-fade-in">
          <div className="mx-auto max-w-xl border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
            ...
            <div className="bg-slate-50 border-t border-slate-100 p-4 text-center text-[9px] text-slate-400 space-y-1">
              ...
              {/* D-08: descadastro funcional — pausa o monitor associado (SOVEREIGN) */}
              <button
                type="button"
                onClick={() => onUnsubscribe && notification.monitorId ? onUnsubscribe(notification.monitorId) : onClose()}
                className="hover:underline cursor-pointer text-slate-500 bg-transparent border-0 p-0 text-[9px]"
              >
                Descadastrar este alerta de e-mail
              </button>
            </div>
          </div>
        </div>
