...
import { Plane, Calendar, Users, DollarSign, Bell, RefreshCw, Trash2, Power, History, Sparkles } from "lucide-react";
...
export default function MonitorCard({ monitor, onScan, onDelete, onToggleStatus }: MonitorCardProps) {
  ...
  const handleScanClick = async () => {
    ...
    const steps = [
      ...
      "Extraindo tarifas vigentes usando processamento semântico do Gemini 3.5...",
      "Processando menor tarifa encontrada..."
    ];

    for (let i = 0; i < steps.length; i++) {
      setScanSteps((prev) => [...prev, steps[i]]);
      // Simulate real step timings
  ...
  // Render SVG Sparkline Graph of Price History
  const renderSparkline = () => {
    const points = [...monitor.history];
    if (monitor.currentPrice) {
      // Append current price if not today
      points.push({
        date: new Date().toISOString(),
        price: monitor.currentPrice,
        site: "atual"
      });
    }
    ...
    if (points.length < 2) {
      return (
        <div className="flex h-24 flex-col items-center justify-center rounded-xl bg-zinc-50 border border-zinc-100 p-2 text-center text-xs text-zinc-400">
          <History className="h-4 w-4 mb-1 text-zinc-300 animate-pulse" />
    ...
    const svgPoints = points.map((p, index) => {
      ...
      // Flip pricing: lower price is at bottom/lower on screen or actually, we want lower path to represent cheaper price (visually standard, so cheap is bottom, expensive is top)
      ...
      <div className="rounded-xl border border-slate-100 bg-slate-50/30 p-3">
        ...
        <div className="relative">
          <svg className="w-full" viewBox={`0 0 ${width} ${height}`} height={height}>
            ...
            {/* Filled area */}
            <path
              d={`M ${padding},${height} L ${svgPoints} L ${width - padding},${height} Z`}
            ...
            {points.map((p, index) => {
              ...
              return (
                <circle
                  ...
                  cy={y}
                  r="3.5"
                  className="fill-white stroke-blue-600 stroke-2 hover:r-5 cursor-help transition-all"
                  title={`R$ ${p.price} (${new Date(p.date).toLocaleDateString()})`}
                />
              );
            })}
          </svg>
  ...
  return (
    <div className={`rounded-2xl border bg-white p-5 shadow-sm transition hover:shadow-md ${
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            {isConfiguredUnderTarget && (
              <span className="flex items-center gap-0.5 rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-bold text-emerald-800 uppercase animate-pulse">
        ...
        <div className="flex items-baseline gap-2 self-start rounded-xl border border-slate-100 bg-slate-50/50 p-2.5">
          <div className="text-right">
            ...
            <span className="text-lg font-black text-slate-850">
              {monitor.currentPrice ? `R$ ${monitor.currentPrice}` : "N/D"}
            </span>
          </div>
          <div className="border-l border-slate-200 pl-2 text-right">
            ...
            <span className="text-xs font-bold text-blue-600">
              R$ {monitor.targetPrice}
            </span>
          </div>
        </div>
      </div>

      {/* Passengers & Selected sites to crawl info */}
      ...
      {/* Scanning loading Console logs inside monitor card */}
      {isScanning && (
        <div className="mt-4 rounded-xl border border-slate-800 bg-slate-900 p-3 font-mono text-[10px] text-blue-400 max-h-36 overflow-y-auto shadow-inner">
          ...
          <div className="space-y-1">
            ...
            {scanResultText && (
              <p className="text-white font-bold mt-1 bg-blue-950 px-1 py-0.5 rounded border border-blue-900">
                ✔ {scanResultText}
              </p>
            )}
          </div>
