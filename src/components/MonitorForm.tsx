...
export default function MonitorForm({ airlineSites, onSubmit, currentUserEmail }: MonitorFormProps) {
  ...
  // Heuristic average market price estimation based on route and passenger counts
  const getAveragePrice = () => {
    const o = (origin || "").toUpperCase().trim();
    const d = (destination || "").toUpperCase().trim();
    if (!o || !d) return 0;

    let basePrice = 1200; // domestic defaults
    ...
    if (isEurope(d) || isEurope(o)) {
      ...
      basePrice = 3500; // generic international
    }
  ...
  // Helper to sync city name automatically
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      // Just some reset defaults or success log
    }
      ...
      <div className="space-y-4">
        ...
        <div>
          <div className="flex justify-between items-center mb-1.5 flex-wrap gap-1">
            <label className="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1.5 animate-fade-in">
            ...
            {avgMarketPrice > 0 && (
              <button
                ...
                className="text-[9px] font-bold text-blue-600 bg-blue-50 border border-blue-100/80 hover:bg-blue-100 hover:text-blue-700 rounded px-2 py-0.5 transition flex items-center gap-1 cursor-pointer animate-fade-in"
              ...
              >
                ...
                <span className="text-[8px] bg-blue-200/50 px-1 py-0.1 rounded text-blue-600 font-bold md:inline hidden">Usar</span>
              </button>
            )}
          </div>
          <div className="relative">
          ...
          <p className="mt-1 text-[10px] text-slate-400 font-medium">
            Enviaremos alerta imediatamente ao alcançar este valor ou menos nos canais.
          </p>
        </div>
        ...
        <button
          ...
          className="w-full mt-2 flex items-center justify-center gap-2 rounded-xl bg-blue-600 hover:bg-blue-750 hover:shadow-lg hover:shadow-blue-100/80 px-4 py-2.5 text-sm font-semibold text-white transition active:scale-[0.99] disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed"
        >
