...
- [005-plan-b-moblix](product/plans/005-plan-b-moblix.md) — Second Plan B candidate spike: Moblix abandoned before an API key was ever obtained — API access sits behind a R$523/month plan with no free/no-card tier, and the trial checkout auto-charges a real card, unlike Travelpayouts' free token
- [006-plan-b-skyscrapper](product/plans/006-plan-b-skyscrapper.md) — Third Plan B candidate spike: Sky Scrapper (RapidAPI, unofficial Skyscanner scraper) confirmed real coverage on FlySpot's canonical gap route (BSB→MIA) on a genuinely free tier — strongest Plan B result so far, but the board's legal-validation requirement before production use is unresolved by this document
