...
- [004-waitlist-landing-root-page](platform/004-waitlist-landing-root-page.md) — Pre-launch waitlist landing (board-requested, since the real search engine and Plan B both remain blocked) renders at `/` for unauthenticated visitors instead of the old redirect-to-`/login`; isolated `mpa_waitlist` collection, honeypot anti-spam, board's 3 non-negotiable copy rules enforced in the component
...
- [004-design-token-theming-architecture](application/004-design-token-theming-architecture.md) — CSS custom properties under `--color-*` in Tailwind v4's `@theme`, dark mode via `prefers-color-scheme` plus a `data-theme` override that always wins; fixed `-solid` token variants for button backgrounds; every color pair verified against WCAG AA in both themes
