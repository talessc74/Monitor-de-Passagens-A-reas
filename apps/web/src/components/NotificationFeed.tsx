...
import { Mail, Clock, Eye, Send } from 'lucide-react';
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-100/30" id="notification-alerts-center">
      <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 font-medium">Histórico de e-mails emitidos</p>
        </div>
        {notifications.length > 0 && (
          <button onClick={onClearAll} className="text-[10px] font-bold text-red-600 hover:text-red-700 uppercase tracking-wider transition">
            Limpar Histórico
          </button>
        )}
      </div>
        ...
        <form onSubmit={handleSendTest} className="space-y-2">
          ...
          <div className="flex items-center justify-between flex-wrap gap-2 pt-1">
            {testSuccessMessage ? (
              ...
              <span className="text-[10px] text-slate-400 font-semibold">Simula o envio imediato.</span>
            )}
            <button
              type="submit"
              disabled={isSendingTest}
              className="flex items-center gap-1 border-0 hover:bg-blue-700 bg-blue-600 font-bold px-3 py-1.5 rounded text-white text-xs whitespace-nowrap shadow-sm shadow-blue-100 transition active:scale-95"
            >
          ...
          notifications.map((notif) => (
            <div
              ...
              className={`flex flex-col gap-2 rounded-xl border p-3.5 transition ${
                notif.type === 'target_reached' ? 'border-emerald-200 bg-emerald-50/10 shadow-emerald-50' : 'border-slate-100 bg-slate-50/20'
              }`}
            >
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`p-1.5 rounded-lg border ${
                      notif.type === 'target_reached' ? 'bg-emerald-100 text-emerald-850 border-emerald-200/50' : 'bg-blue-100 text-blue-800 border-blue-200/50'
                    }`}
                  >
              ...
              <p className="text-xs text-slate-650 font-medium leading-relaxed pl-2 border-l-2 border-slate-200 py-0.5">{notif.message}</p>
            </div>
          ))
        )}
      </div>
